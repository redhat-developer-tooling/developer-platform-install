  ...
  for (let key in reqs) {
    if ((reqs[key].useDownload == undefined || reqs[key].useDownload) && reqs[key].bundle === bundle && (reqs[key].fileName || reqs[key].file)) {
      ...
      if(reqs[key].fileName) {
        files[key] = {
          url: reqs[key].url,
          fileName: path.basename(reqs[key].fileName),
          sha256sum: reqs[key].sha256sum,
          size: reqs[key].size
        };
      } else {
        files = reqs[key].file;
      }
      // convert from object to array of fields
      files = Object.keys(files).map(function(file) {
        return files[file];
      });
      files.forEach((file) =>{
        let currentFile = path.join(targetFolder, file.fileName);
        promises.push(() => {
          return new Promise((resolve, reject) => {
            console.log(file);
            // if file is already downloaded, check its sha against the stored one
            downloadAndReadSHA256(targetFolder, file.fileName + '.sha256', file.sha256sum, reject, (currentSHA256) => {
              ...
              comm.isExistingSHA256Current(currentFile, currentSHA256, (dl) => {
                if(dl) {
                  ...
                  downloadFileAndCreateSha256(targetFolder, file.fileName, file.url, currentSHA256, resolve, reject);
                }
              });
            });
          });
