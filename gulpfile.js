...
var buildFolderRoot = 'dist/win/' + nowFolderName() +'/';
          ...
          downloadAndReadSHA256(targetFolder, key + ".sha256", reqs[key].sha256sum, reject, (currentSHA256) => {
            ...
            isExistingSHA256Current(currentFile, currentSHA256, (dl) => {
              if(dl) {
                ...
                downloadFileAndCreateSha256(targetFolder, key, reqs[key].url, reqs[key].sha256sum, resolve, reject);
              }
            });
          });
        })
...
function nowFolderName(){
  ...
  ds = ds.substring(0,ds.indexOf('.')).replace(/T/g,'_').replace(/:/g,'-');
  return ds;
};
